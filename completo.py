...
try:
    ...
    st.warning("Logo non trovato. Carica 'logo_nicefit.png' nella root del repository.")
...
if scelta == "Dashboard":
    ...
    if not df_cont.empty:
        ...
        ax = df_plot.plot(kind="bar", stacked=True, color=["#5cb85c","#d9534f"], edgecolor="black")
        plt.ylabel("Importo (€)")
        plt.title("Incassi (verde) e Pagamenti (rosso) per palestra")
        st.pyplot(plt.gcf())
        plt.clf()
    else:
...
# -------------------------------
    ...
    ricerca = st.text_input("🔍 Cerca fornitore")
    if ricerca:
...
# -------------------------------
    ...
    st.dataframe(df.style.applymap(lambda v: f'background-color:{PALESTRE_COLORI.get(v,"#FFF")}; color:black', subset=["palestra"]), use_container_width=True)
